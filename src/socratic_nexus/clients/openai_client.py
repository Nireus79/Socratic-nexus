"""
OpenAI API client for Socrates AI

Provides both synchronous and asynchronous interfaces for calling OpenAI API,
...
class OpenAIClient:
    ...
    def __init__(
        ...
        Args:
            api_key: OpenAI API key (optional - can be None for API server mode using database keys)
            orchestrator: Reference to AgentOrchestrator for event emission and token tracking
            subscription_token: Optional - OpenAI subscription token for subscription-based auth
        """
        self.api_key = api_key
        self.subscription_token = subscription_token
        self.orchestrator = orchestrator
        self.model = orchestrator.config.openai_model if orchestrator else "gpt-4-turbo"
        self.logger = logging.getLogger("socrates.clients.openai")
        ...
        if subscription_token:
            try:
                self.subscription_client = openai.OpenAI(api_key=subscription_token)
                self.subscription_async_client = openai.AsyncOpenAI(api_key=subscription_token)
                self.logger.info("Subscription-based clients initialized")
            except Exception as e:
                self.logger.warning(f"Failed to initialize subscription clients: {e}")

        # Cache for insights extraction to avoid redundant OpenAI API calls
        # Maps message hash -> extracted insights
        self._insights_cache: Dict[str, Dict[str, Any]] = {}

        # Cache for question generation to avoid redundant OpenAI API calls
        ...
        if user_auth_method == "subscription":
            if not self.subscription_token:
                raise ValueError(
                    ...
                    "Set OPENAI_SUBSCRIPTION_TOKEN environment variable."
                )
    ...
    def _get_user_api_key(self, user_id: Optional[str] = None) -> tuple:
        ...
        """
        if user_id:
            stored_key = self.orchestrator.database.get_api_key(user_id, "openai")
            if stored_key:
    ...
    def _get_client(self, user_auth_method: str = "api_key", user_id: Optional[str] = None):
        ...
        try:
            ...
            if api_key and not api_key.startswith("placeholder"):
                ...
                # No valid key found
                raise APIError(
                    "No API key configured. Please set your API key in Settings > LLM > OpenAI",
    ...
    def extract_insights(
        ...
        """
        Extract insights from user response using OpenAI (synchronous) with caching.
        ...
        # Check cache first to avoid redundant OpenAI API calls
        ...
        try:
            ...
            insights = self._parse_json_response(response.choices[0].message.content.strip())
...
- If you have explanations, put them ONLY in docstrings
    ...
    def generate_socratic_question(
        ...
        """
        Generate a Socratic question using OpenAI with optional caching.
    ...
    def generate_response(
        ...
        """
        Generate a general response from OpenAI for any prompt.
    ...
    async def generate_response_async(
        ...
        """
        Generate a general response from OpenAI asynchronously.
    ...
    def test_connection(self, user_auth_method: str = "api_key") -> bool:
        ...
        try:
            ...
            self.logger.warning("OpenAI API connection test skipped - no valid API key configured")
            return False
        except Exception as e:
            self.logger.error(f"OpenAI API connection test failed: {e}")
            raise APIError(
                f"Failed to connect to OpenAI API: {e}", error_type="CONNECTION_ERROR"
            ) from e
    ...
    def _calculate_cost(self, usage: Any) -> float:
        ...
        # OpenAI GPT-4 Turbo pricing (approximate - check pricing page for latest)
    ...
    def _parse_json_response(self, response_text: str) -> Any:
        """Parse JSON from OpenAI response with error handling. Returns dict or list."""
        try:
